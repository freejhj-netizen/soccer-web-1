...
const Free: React.FC = () => {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      const postsData = querySnapshot.docs
        ...
        .filter((post: any) => post.type === 'free') as Post[];
      setPosts(postsData);
    } catch (error) {
  ...
  const handleEdit = (post: Post) => {
    if (userData?.uid !== post.authorUid && userData?.role !== 'admin') return;
    setEditingPost(post);
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-sm font-semibold truncate pr-2 mb-1 text-white">{post.title}</div>
